...
from decision_transformer.dt.networks.networks import AutonomousGRU, MLP, Transformer, FeedForwardModel, sinusoidal_pos_emb, mish
...
class VAE(nn.Module):
    ...
    def __call__(self, ts, s_t, z_t, a_t, y_t, rtg_t, horizon, mask, dynamics_apply, dynamics_params, key):
        ...
        def get_H_step(y_t, logits, horizon, key):

            logits_trunc = logits.copy()
            # logits_trunc = logits_trunc.at[horizon:].set(-1e30)
            mask = jnp.arange(self.context_len) >= horizon  # shape (N,)
            logits_trunc = jnp.where(mask, -1e30, logits_trunc)
        ...
        batch_get_H_step = jax.vmap(get_H_step, in_axes=(0, None, 0, 0))
...
class empowerment(nn.Module):
    ...
    def __call__(self, ts, s_t, z_t, a_t, y_t, rtg_t, horizon, mask, train_precoder, dynamics_apply, dynamics_params, key):
        ...
        sample_z_key, sample_i_key, sample_y_key, dropout_key, encoder_key = jax.random.split(key, 5)
        ...
        if self.alternate_training:
            ...
            actions = jax.lax.cond(train_precoder,
                                apply_with_grad,
                                apply_without_grad,
                                self.variables['params']['precoder'], s_t, z_samp)
            ...
            def sample_one_traj(s_t, actions, key, dynamics_params):
                ...
                def peform_rollout(state, key, actions, dynamics_params):
                    ...
                    def step_fn(carry, action):
                        ...
                        al_std = jnp.clip(jnp.sqrt(jnp.square(jnp.ones_like(s_log_std) * 1e-3).mean(0)), min=1e-3)
...
class empowerment_MC(nn.Module):
    state_dim: int
    act_dim: int
    controlled_variables_dim: int
    controlled_variables: List
    ...
    def __call__(self, ts, s_t, z_t, a_t, y_t, rtg_t, horizon, mask, train_precoder, dynamics_apply, dynamics_params, key):
        ...
        sample_z_keys = jax.random.split(sample_z_key, self.num_MC_samples)
        z_samp = jax.vmap(source_dist.sample)(seed=sample_z_keys)

        ###################### precode ###################### 

        actions = jax.vmap(self.precoder, in_axes=(None,0))(s_t, z_samp)
        ...
        def sample_one_traj(s_t, actions, key, dynamics_params):
            ...
            def peform_rollout(state, key, actions, dynamics_params):
                ...
                def step_fn(carry, action):
                    ...
                    delta_s_dist = tfd.TransformedDistribution(distribution=s_base_dist, bijector=bounded_bijector)
                
                    s_base_dist_all = tfd.MultivariateNormalDiag(loc=s_mean, scale_diag=jnp.exp(s_log_std))
                    delta_s_dist_all = tfd.TransformedDistribution(distribution=s_base_dist_all, bijector=bounded_bijector)

                    delta_s = delta_s_dist.sample(seed=sample_s_key)
                    delta_s = delta_s * self.delta_obs_scale + self.delta_obs_shift
                    ...
                    carry = next_state, key, dynamics_params
                    return carry, (s_next, info_gain, delta_s_dist_all)

                carry = state, key, dynamics_params
                _, (next_state, info_gain, delta_s_dist) = jax.lax.scan(step_fn, carry, actions)
                
                return next_state, info_gain, delta_s_dist
            
            batch_peform_rollout = jax.vmap(peform_rollout, in_axes=(0,0,0,None))

            dynamics_keys = jax.random.split(key, actions.shape[0])
            next_state, info_gain, delta_s_dist = batch_peform_rollout(s_t[:,0,:], dynamics_keys, actions, dynamics_params)

            # y_samp = jnp.take_along_axis(next_state, horizon[..., None]-1, axis=1)[...,self.controlled_variables]
            y_samp = next_state[...,self.controlled_variables]

            return y_samp, info_gain, delta_s_dist

        batch_sample_one_traj = jax.vmap(sample_one_traj, in_axes=(None,0,None,None))

        y_samp, info_gain, delta_s_dist = batch_sample_one_traj(s_t, actions, key, dynamics_params) # N_MC x B x T x D
