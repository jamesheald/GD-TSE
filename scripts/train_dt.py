...
from decision_transformer.dt.networks.networks import dynamics, AutonomousGRU, MLP, Transformer, MLP_precoder
from decision_transformer.dt.models.models import make_transformer_networks, VAE, empowerment, empowerment_MC
...
def train(args):
    ...
    def eval_model(model, params, iter, loop='open'):
        ...
        for rollout in range(n_rollouts):
            ...
            # wandb.log({
            # f"video/{loop}_{model}_{iter}_{rollout}": wandb.Video(
            #     np.stack(frames), fps=30, format="mp4"
            # )
            # })
    ...
    if args.resume_start_time_str is not None and args.resume_dynamics is False:
        ...
        def dynamics_loss(dynamics_params: Any,
            ...
            if args.Markov_dynamics:
                ...
                y_p = dynamics_model.apply(dynamics_params, ts, s_tm1_s_t, dummy_z_t, a_t, y_t, rtg_t, horizon, deterministic=False, rngs={'dropout': key})
    ...
    def emp_loss(emp_params: Any,
        ...
        loss, info_gain_loss = emp_model.apply(emp_params, ts, s_tm1_s_t, dummy_z_t, a_t, y_t, rtg_t, horizon, mask, train_precoder, dynamics_model.apply, _dynamics_params, emp_key, rngs={'dropout': dropout_key})

        return loss + info_gain_loss * args.info_gain_weight, (loss, info_gain_loss * args.info_gain_weight)

    # emp_grad = jax.jit(jax.value_and_grad(emp_loss), static_argnames=('train_precoder'))
    emp_grad = jax.jit(jax.value_and_grad(emp_loss, has_aux=True))
    ...
    def update_step_emp(
        ...
        (loss, (precoder_loss, info_gain_loss)), emp_grads = emp_grad(state.params, transitions, key_emp, train_precoder)
    ...
    for i_train_iter in range(max_train_iters):
        ...
        log_precoder_losses.append(training_metrics['precoder_loss'])
        log_info_gain_losses.append(training_metrics['info_gain_loss'])

        mean_emp_loss = np.mean(log_emp_losses)
        mean_precoder_loss = np.mean(log_precoder_losses)
        mean_info_gain_loss = np.mean(log_info_gain_losses)
        time_elapsed = str(datetime.now().replace(microsecond=0) - start_time)
        ...
        wandb.log({'mean_emp_loss': mean_emp_loss,
                   'mean_precoder_loss': mean_precoder_loss,
                   'mean_info_gain_loss': mean_info_gain_loss})
...
if __name__ == "__main__":
    ...
    parser.add_argument('--n_dynamics_ensembles', type=int, default=5)
    ...
    parser.add_argument('--Markov_dynamics', action='store_false')

    parser.add_argument('--dynamics_batch_size', type=int, default=4096)
    parser.add_argument('--vae_batch_size', type=int, default=1024)
    parser.add_argument('--emp_batch_size', type=int, default=1024)
    ...
    parser.add_argument('--encoder_dropout_rates', type=List, default=[0.,0.])

    parser.add_argument('--max_train_iters', type=int, default=500)
    parser.add_argument('--num_updates_per_iter', type=int, default=100)
    parser.add_argument('--dynamics_save_iters', type=int, default=250)
    parser.add_argument('--vae_save_iters', type=int, default=100)
    parser.add_argument('--emp_save_iters', type=int, default=50)
    ...
    parser.add_argument('--state_dep_prior', action='store_true')
    parser.add_argument('--trajectory_version', action='store_true')
    parser.add_argument('--uncertainty_weight', type=float, default=0.)
    parser.add_argument('--info_gain_weight', type=float, default=1.)
    parser.add_argument('--y_decoder_weight', type=float, default=0.)
    
    parser.add_argument('--resume_dynamics', action='store_true')
    parser.add_argument('--resume_vae', action='store_true')
    parser.add_argument('--resume_start_time_str', type=str, default='25-07-13-14-40-38') # None,  '25-06-25-16-17-25'
